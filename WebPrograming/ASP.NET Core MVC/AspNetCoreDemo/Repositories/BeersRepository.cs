...
namespace AspNetCoreDemo.Repositories
{
	public class BeersRepository : IBeersRepository
	{
		...
		{
			return GetBeers().ToList();
		}

		public Beer GetById(int id)
		{
			Beer beer = GetBeers().FirstOrDefault(b => b.Id == id);
		...
		{
			Beer beer = GetBeers().FirstOrDefault(b => b.Name == name);
		...
		{
			IQueryable<Beer> result = GetBeers();
        ...
        {
            var beerToDelete = context.Beers.FirstOrDefault(beer => beer.Id == id)
                ?? throw new EntityNotFoundException($"Beer with id {id} not found");
            context.Beers.Remove(beerToDelete);
        ...
        {
            return context.Beers.Any(beer => beer.Name == name);
        }
    }
}
